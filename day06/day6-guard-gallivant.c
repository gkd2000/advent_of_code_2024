...
int dirsToInt(int rowDir, int colDir);
bool runSimulation(int startRow, int startCol);
...
int main (int argc, char **argv)
{
    ...
    while (isInBounds(guardRow, guardCol))
    {
        ...
        if (!hasTurned && (RoomGrid[guardRow + rowDir][guardCol + colDir] != '#') && runSimulation(guardRow, guardCol))
        {
...
bool runSimulation(int startRow, int startCol)
{
    ...
    simulationRoomGrid[startRow-1][startCol] = '#';
    int currentDir;
    ...
    while (isInBounds(guardRow, guardCol) && !foundCycle)
    {
        ...
        if (simulationRoomGrid[guardRow][guardCol] == '.' ||
            simulationRoomGrid[guardRow][guardCol] == '^' ||
            simulationRoomGrid[guardRow][guardCol] == 'X')
        {
